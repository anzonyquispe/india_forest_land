...
ppool_syn <- multisynth(per_treecover ~ post_ror_data_entry, 
                        vill_id, year, df2_avobe, time_cohort = TRUE, n_leads = 9 )

filename <- paste0( "avobe_median_all.RDS")
...
filename <- paste0( "avobe_median_all_sum.RDS")
path <- file.path("E_Estimates/_vcf_10_augsyth/median_filter", filename)
...
filename <- paste0( "avobe_median_all_plot.png")
...
filename <- paste0( "below_median_all.RDS")
path <- file.path("E_Estimates/_vcf_10_augsyth/median_filter", filename)
saveRDS( ppool_syn, path )
...
# Get Summary result
ppool_syn_time_summ <- summary( ppool_syn )
filename <- paste0( "below_median_all_sum.RDS")
path <- file.path("E_Estimates/_vcf_10_augsyth/median_filter", filename)
saveRDS( ppool_syn_time_summ,  path )
...
filename <- paste0( "below_median_all_plot.png")
path <- file.path("F_Figures/_vcf_10_augsyth/state", filename)
ggsave(filename = path, plot = p, width = 6, height = 4, dpi = 300)
