...
ppool_syn <- multisynth(per_treecover ~ post_ror_data_entry, 
                        vill_id, year, df2_sch, time_cohort = TRUE, n_leads = 9 )

filename <- paste0( "sch_median_all.RDS")
...
filename <- paste0( "sch_median_all_sum.RDS")
path <- file.path("E_Estimates/_vcf_10_augsyth/sch", filename)
...
filename <- paste0( "sch_median_all_plot.png")
...
filename <- paste0( "nonsch_median_all.RDS")
path <- file.path("E_Estimates/_vcf_10_augsyth/sch", filename)
saveRDS( ppool_syn, path )
...
# Get Summary result
ppool_syn_time_summ <- summary( ppool_syn )
filename <- paste0( "nonsch_median_all_sum.RDS")
path <- file.path("E_Estimates/_vcf_10_augsyth/sch", filename)
saveRDS( ppool_syn_time_summ,  path )
...
filename <- paste0( "nonsch_median_all_plot.png")
path <- file.path("F_Figures/_vcf_10_augsyth/sch", filename)
ggsave(filename = path, plot = p, width = 6, height = 4, dpi = 300)
