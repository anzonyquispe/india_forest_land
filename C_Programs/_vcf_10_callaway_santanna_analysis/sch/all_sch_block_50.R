...
filename <- paste0( "sch_median_block_50.RDS")
...
n_df <- df2_sch[t2ev_ror_data_entry_block_50 %in% est_df$Time][, .N, by = t2ev_ror_data_entry_block_50]
...
p <- ggplot(est_df, aes(x = Time, y = Estimate)) + 
  ...
  ggtitle( "Ror Data Entry at Block Level when 50% \nVillages are Treated (Sch States)" ) +
  theme_minimal()

filename <- paste0( "sch_median_block_50_plot.png")
path <- file.path("F_Figures/_vcf_10_callaway_santanna_analysis/sch", 
                  filename)
...
filename <- paste0( "nonsch_median_block_50.RDS")
path <- file.path("E_Estimates/_vcf_10_callaway_santanna_analysis/sch", 
                  filename)
saveRDS( out, path )
...
# Getting the number of obseravtions
n_df <- df2_nonsch[t2ev_ror_data_entry_block_50 %in% est_df$Time][, .N, by = t2ev_ror_data_entry_block_50]
n_df$Time <- n_df$t2ev_ror_data_entry_block_50
setorder(n_df,Time )
setorder(est_df,Time )
...
p <- ggplot(est_df, aes(x = Time, y = Estimate)) + 
  ...
  geom_vline(aes(xintercept = 0), linetype = "dashed", color = "gray") +
  geom_hline(aes(yintercept = 0), linetype = "dashed", color = "gray") + 
  ggtitle( "Ror Data Entry at Block Level when 50% \nVillages are Treated (Nonsch States)" ) +
  theme_minimal()

filename <- paste0( "nonsch_median_block_50_plot.png")
path <- file.path("F_Figures/_vcf_10_callaway_santanna_analysis/sch", 
                  filename)
ggsave(filename = path, plot = p, width = 6, height = 4, dpi = 300)
