...
filename <- paste0( "sch_median_block_any.RDS")
...
p <- ggplot(est_df, aes(x = Time, y = Estimate)) + 
  ...
  ggtitle( "Ror Data Entry at Block Level when Any \nVillage is Treated (Sch States)" ) +
  theme_minimal()

filename <- paste0( "sch_median_block_any_plot.png")
...
filename <- paste0( "nonsch_median_block_any.RDS")
path <- file.path("E_Estimates/_vcf_10_callaway_santanna_analysis/sch", 
                  filename)
saveRDS( out, path )
...
p <- ggplot(est_df, aes(x = Time, y = Estimate)) + 
  ...
  geom_vline(aes(xintercept = 0), linetype = "dashed", color = "gray") +
  geom_hline(aes(yintercept = 0), linetype = "dashed", color = "gray") + 
  ggtitle( "Ror Data Entry at Block Level when Any \nVillage is Treated (Nonsch States)" ) +
  theme_minimal()

filename <- paste0( "nonsch_median_block_any_plot.png")
path <- file.path("F_Figures/_vcf_10_callaway_santanna_analysis/sch", 
                  filename)
ggsave(filename = path, plot = p, width = 6, height = 4, dpi = 300)
